...
const EditingDialog = (props) => {
  const titleClasses = ['title', ...(props.titleClasses || [])];
      ...
      <div className='h5p-editing-dialog'>
        <div className='h5p-editing-dialog-header'>
          ...
          <div className='h5p-editing-dialog-button-row'>
            <button className='remove-button' onClick={props.removeAction.bind(this)}>
              {props.removeLabel}
            </button>
            <button className='done-button' onClick={props.doneAction.bind(this)}>
              {props.doneLabel}
            </button>
          </div>
        </div>
        <div className='h5p-editing-dialog-body'>{props.children}</div>
      </div>
    </div>
  );
};
